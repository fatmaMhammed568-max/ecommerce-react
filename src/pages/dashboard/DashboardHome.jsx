...
import dashboardService from '../../services/dashboardService';  // ✅ نقطتين فقط (مش تلاتة)
import Loader from '../../components/common/Loader';            // ✅ نقطتين فقط
...
const DashboardHome = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // ✅ جلب آخر الطلبات من API
      const ordersResponse = await dashboardService.getRecentOrders();
      if (ordersResponse?.status) {
        setRecentOrders(ordersResponse.data || []);
      }

      // ✅ جلب أكثر المنتجات مبيعاً من API
      const productsResponse = await dashboardService.getBestSellers();
      if (productsResponse?.status) {
        setBestSellers(productsResponse.data || []);
      }

      // ✅ جلب آخر المستخدمين من API
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <Loader text="جاري تحميل لوحة التحكم..." />
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              bestSellers.map((product) => (
                <div key={product.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  <div className="flex items-center gap-3">
                    <img
                      src={product.main_image || 'https://via.placeholder.com/50x50'}
                      alt={product.name}
                      className="w-12 h-12 object-cover rounded-lg border border-gray-200"
                      onError={(e) => {
                        e.target.src = 'https://via.placeholder.com/50x50/0284c7/ffffff?text=Product';
                      }}
                    />
                    <div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            {recentUsers.length > 0 ? (
              recentUsers.map((user) => (
                ...
                >
                  ...
                  {user.role === 1 && (
                    <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-full">مدير</span>
                  )}
                </Link>
              ))
            ) : (
          ...
          <Link 
            to="/dashboard/users" 
            className="mt-4 text-primary-600 hover:text-primary-700 text-sm font-medium flex items-center justify-center gap-1 transition-colors"
          >
